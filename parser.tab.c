...
{
       0,   123,   123,   125,   126,   127,   128,   129,   131,   132,
     133,   135,   136,   137,   138,   139,   142,   143,   144,   146,
     147,   149,   150,   151,   152,   153,   154,   156,   157,   159,
     160,   163,   164,   167,   168,   170,   193,   199,   200,   201,
     204,   205,   208,   209,   213,   221,   228,   235,   242,   249,
     256,   257,   263,   264,   270,   272,   273,   274,   275,   276,
     277,   278,   279,   280,   281,   282,   284,   287,   288,   289,
     294,   295,   297,   298,   299,   300,   301,   303,   307,   308,
     309,   311,   312,   314,   315,   318,   319,   320,   321,   322,
     323,   324,   326,   327,   328,   331,   332,   333,   334,   335
};
#endif
...
#line 171 "parser.y"
                {
		int tipo_variable;
		if (strcasecmp((yyvsp[-1].strval),"entero")==0){
			tipo_variable = ENTERO;
		} else if (strcasecmp((yyvsp[-1].strval),"real")==0) {
			tipo_variable = REAL;
		} else if (strcasecmp((yyvsp[-1].strval),"booleano")==0) {
			tipo_variable = BOOLEANO;
	    } else if (strcasecmp((yyvsp[-1].strval),"caracter")==0) {
			tipo_variable = CARACTER;
	    } else if (strcasecmp((yyvsp[-1].strval),"cadena")==0) {
		...
		nuevo_simbolo(tabla_simbolos, (yyvsp[-3].strval), VARIABLE, tipo_variable);
...
#line 193 "parser.y"
                                                {
        nuevo_simbolo(tabla_simbolos, (yyvsp[-2].strval), VARIABLE, (yyvsp[0].intval));
...
#line 209 "parser.y"
               {
    	dir_elemento* exp1 = nuevo_dir_elemento_celda_TS((yyvsp[0].simval));
    	(yyval.dirval) = exp1;
    }
...
#line 213 "parser.y"
	...
	gen(tabla_cuadruplas, (yyvsp[-1].intval), exp1, exp2, sim_temporal);
